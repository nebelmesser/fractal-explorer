...
function poolSize(): number {
  ...
  return Math.max(1, Math.min(MAGNET_CPU_WORKERS, cores - 1));
}

export function magnetsCpuConcurrency(): number {
  return poolSize();
}
...
function onMessage(event: MessageEvent<MagnetsTileResponse>): void {
  const job = pending.get(event.data.id);
  if (!job) return;
  pending.delete(event.data.id);
  if (!event.data.buffer || event.data.error) {
    job.reject(new Error(event.data.error ?? 'Magnets CPU worker returned no buffer'));
    return;
  }
...
function spawnWorker(): Worker {
  ...
  worker.onerror = (event) => failWorker(worker, new Error(event.message || 'Magnets CPU worker failed'));
  worker.onmessageerror = () => failWorker(worker, new Error('Magnets CPU worker message error'));
  return worker;
}
...
export function cancelMagnetsCpu(): void {
  if (!pool) return;
  cancelEpoch += 1;
  const obsolete = new Set(pool);
  pool = [];
  idle.length = 0;
  const error = new Error('Magnets CPU tile superseded by a newer camera');
...
function run(worker: Worker, request: MagnetsTileRequest): Promise<ArrayBuffer> {
...
export async function fillMagnetsTile(
  view: ViewRect,
  width: number,
  height: number,
  params: MapParams,
  ...
  try {
    if (epoch !== cancelEpoch) {
      const error = new Error('Magnets CPU tile superseded before dispatch');
      error.name = 'AbortError';
      throw error;
    }
    const buffer = await run(worker, {
      ...
      H: params.H ?? MAGNET_H,
      R: params.R ?? MAGNET_R,
      DT: params.DT ?? MAGNET_DT,
      maxIter: Math.max(
        MAGNET_MIN_ITER,
        Math.round(params.MAX_ITERATIONS ?? MAGNET_MIN_ITER),
      ),
    });
