...
`;function me(e){return new Worker(""+new URL("tile-worker-Bhn0iWqD.js",import.meta.url).href,{type:"module",name:e?.name})}let f=null,de=1,C=0;const x=new Map,g=[],H=[];function b(){const e=typeof navigator>"u"?4:navigator.hardwareConcurrency??4;return Math.max(1,Math.min(Z,e-1))}function xe(){return b()}function _e(){const e=g.pop();return e?Promise.resolve(e):new Promise(n=>H.push(n))}function R(e){if(!f?.includes(e))return;const n=H.shift();n?n(e):g.push(e)}function ge(e){const n=x.get(e.data.id);if(n){if(x.delete(e.data.id),!e.data.buffer||e.data.error){n.reject(new Error(e.data.error??"Chirikov CPU worker returned no buffer"));return}n.resolve(e.data.buffer)}}function A(){const e=new me;return e.onmessage=ge,e.onerror=n=>S(e,new Error(n.message||"Chirikov CPU worker failed")),e.onmessageerror=()=>S(e,new Error("Chirikov CPU worker message error")),e}function S(e,n){for(const[i,a]of x)a.worker===e&&(x.delete(i),a.reject(n));const o=g.indexOf(e);if(o>=0&&g.splice(o,1),!f)return;const t=f.indexOf(e);if(t<0)return;f.splice(t,1);try{e.terminate()}catch{}const r=A();f.push(r),R(r)}function ye(){if(!f)return;C+=1;const e=new Set(f);f=[],g.length=0;const n=new Error("Chirikov CPU tile superseded by a newer camera");n.name="AbortError";for(const[o,t]of x)e.has(t.worker)&&(x.delete(o),t.reject(n));for(const o of e)try{o.terminate()}catch{}for(let o=0;o<b();o++){const t=A();f.push(t),R(t)}}function Me(){if(!f){f=[];for(let e=0;e<b();e++){const n=A();f.push(n),g.push(n)}}}function we(e,n){return new Promise((o,t)=>{x.set(n.id,{worker:e,resolve:o,reject:t}),e.postMessage(n)})}async function ve(e,n,o,t){Me();const r=C,i=await _e();try{if(r!==C){const c=new Error("Chirikov CPU tile superseded before dispatch");throw c.name="AbortError",c}const a=await we(i,{id:de++,width:Math.max(1,Math.round(n)),height:Math.max(1,Math.round(o)),xMin:e.xMin,xMax:e.xMax,yMin:e.yMin,yMax:e.yMax,k:t.K??k,iterations:Math.max(v,Math.round(t.MAX_ITERATIONS??I))});return new Float32Array(a)}finally{R(i)}}function ke(e,n,o,t,r){const i=new ArrayBuffer(80),a=new Float32Array(i),c=new Uint32Array(i);a[0]=e.xMin,a[1]=e.xMax,a[2]=e.yMin,a[3]=e.yMax,a[4]=t.K??k,c[12]=Math.max(v,Math.round(t.MAX_ITERATIONS??I)),c[13]=n,c[14]=o,c[15]=0,c[16]=0;const h=L(e,r.normView??e,n,o);return c[17]=h.y0<<16|h.x0,c[18]=h.y1<<16|h.x1,c[19]=1,i}const Ie={computeWgsl:pe,entryPoint:"simulate",uniformBytes:80,tileSize:128,dispatchBatch:1,packUniforms:ke},Ce={id:"chirikov",title:"Chirikov–Taylor map",settledResolution:{gpu:"device",cpu:"device"},defaultView:{xMin:-y,xMax:y,yMin:-y,yMax:y},navigation:{xPeriod:{period:_,center:0},yPeriod:{period:_,center:0}},workBudget:{param:"MAX_ITERATIONS",min:v,max:E,step:K,adaptive:!1},params:[{key:"K",label:"K",kind:"float",min:G,max:J,step:Q,default:k,digits:3,section:"primary"},{key:"MAX_ITERATIONS",label:"N",kind:"int",min:v,max:E,step:K,default:I,section:"primary"}],gpu:Ie,cpu:{fillTile:ve,cancelPending:ye,concurrency:xe(),handoffPx:.5}};async function be(){await F(),await $(Ce,fe),D(),X()}be();
