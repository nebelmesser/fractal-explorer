import{v as H,a as W,b as D,s as X,n as $,l as q,c as j,d as G,e as K}from"./menu-DEf-9Ab7.js";function U(e){return getComputedStyle(document.documentElement).getPropertyValue(e).trim()}function J(e,t){const o=e/Math.max(2,Math.floor(t/90)),n=10**Math.floor(Math.log10(Math.max(o,Number.MIN_VALUE))),r=o/n;return(r<=1?1:r<=2?2:r<=5?5:10)*n}function Q(e,t){const o=Math.max(0,Math.min(8,-Math.floor(Math.log10(t)))),n=e.toFixed(o),r=o>0?n.replace(/\.?0+$/,""):n;return r==="-0"?"0":r.replace("-","−")}function S(e,t,o){const n=t-e;if(!(n>0))return[];const r=J(n,o),a=Math.ceil(e/r-1e-10)*r,l=[];for(let s=a;s<=t+r*1e-10&&l.length<64;s+=r)l.push({value:s,label:Q(s,r)});return l}function O(e,t){const o=Array.from(e.children);for(let n=0;n<t.length;n++){const r=t[n],a=o[n]??document.createElement("span");o[n]||e.append(a),a.textContent=r.textContent,a.style.left=r.style.left,a.style.top=r.style.top}for(let n=o.length-1;n>=t.length;n--)o[n].remove()}function Z(e,t,o,n){const r=e.clientWidth,a=e.clientHeight;if(r<8||a<8)return;const l=Math.min(window.devicePixelRatio||1,2),s=Math.round(r*l),c=Math.round(a*l);(e.width!==s||e.height!==c)&&(e.width=s,e.height=c);const i=e.getContext("2d");if(!i)return;i.setTransform(l,0,0,l,0,0),i.clearRect(0,0,r,a),i.strokeStyle=U("--lyapunov-axis"),i.shadowColor=U("--shadow"),i.shadowBlur=4,i.lineWidth=1;const p=[];for(const f of S(t.xMin,t.xMax,r)){const u=(f.value-t.xMin)/H(t)*r;if(u<=2||u>=r-2||(i.beginPath(),i.moveTo(u,a),i.lineTo(u,a-6),i.stroke(),u<28||u>r-28))continue;const y=document.createElement("span");y.textContent=f.label,y.style.left=`${u}px`,p.push(y)}O(o,p);const m=[];for(const f of S(t.yMin,t.yMax,a)){const u=(f.value-t.yMin)/W(t)*a;if(u<=2||u>=a-2||(i.beginPath(),i.moveTo(r,u),i.lineTo(r-6,u),i.stroke(),u<16||u>a-18))continue;const y=document.createElement("span");y.textContent=f.label,y.style.top=`${u}px`,m.push(y)}O(n,m)}const R=2,B=4,h=["AABAB","AB","AAB","ABB","AABB"],g=0;h[g];const A=.5,Y=200,T=1e-7,F=-Math.log(T),b=200,C=1600,I=40,ee=480,te=4;function ne(e){const t=Math.max(0,Math.min(h.length-1,Math.round(e)));return h[t]??h[g]}function oe(e,t,o,n=g,r=A){const a=ne(n),l=a.length;let s=r;const c=Math.max(1,Math.floor(Y/l));for(let m=0;m<c;m++){for(let f=0;f<l;f++)s=(a.charCodeAt(f)===66?t:e)*s*(1-s);if(!Number.isFinite(s)||Math.abs(s)>16)return 1.5}const i=Math.max(1,Math.floor(Math.round(o)/l));let p=0;for(let m=0;m<i;m++){for(let f=0;f<l;f++){const u=a.charCodeAt(f)===66?t:e;p+=Math.log(Math.max(T,Math.abs(u*(1-2*s)))),s=u*s*(1-s)}if(!Number.isFinite(s)||Math.abs(s)>16)return 1.5}return p/(i*l)}function re(e){return e-F}const ae=`// Gold marks negative (stable) exponents; blue marks positive (chaotic)
...
`;function w(e){return Math.min(1,Math.max(0,e))}function x(e,t,o){const n=w((o-e)/(t-e));return n*n*(3-2*n)}function v(e,t,o){return[e[0]+(t[0]-e[0])*o,e[1]+(t[1]-e[1])*o,e[2]+(t[2]-e[2])*o]}function se(e){let t=v([.01,.006,.002],[.72,.27,.005],x(.02,.52,e));return t=v(t,[1,.68,.035],x(.4,.82,e)),v(t,[1,.965,.68],x(.84,1,e))}function ie(e){let t=v([.002,.006,.018],[.018,.075,.21],x(0,.36,e));return t=v(t,[.055,.25,.62],x(.22,.76,e)),v(t,[.27,.5,.83],x(.72,1,e))}function le(e,t,o){if(!Number.isFinite(e))return[0,0,0];let r=e<0?w((-e/2.4)**.42):w((e/.72)**.46),a=x(0,.018,Math.abs(e));if(o){const s=Math.max(1e-6,o.hi-o.lo),c=(Math.log1p(-Math.log(1e-7))-o.lo)/s,i=e<0?w((c-t)/Math.max(.08,c)):w((t-c)/Math.max(.08,1-c)),p=1-x(.025,.18,s),m=i**.3;r+=(m-r)*p*.9;const f=x(.006,.045,i);a+=(f-a)*p}const l=e<0?se(r):ie(r);return[l[0]*a,l[1]*a,l[2]*a]}const ce={gpuWgsl:ae,colorize(e,t,o,n){const r=le(re(e),t,n);return o?[1-r[0],1-r[1],1-r[2]]:r},exposure:"quantile",initialMedian:1};function E(e){const t=document.getElementById(e);if(!t)throw new Error(`Lyapunov presentation is missing #${e}`);return t}function ue(e){return`${e<0?"−":"+"}${Math.abs(e).toFixed(4)}`}function pe(e){const t=Math.max(0,Math.min(h.length-1,Math.round(e.RHYTHM??g)));return h[t]}function fe(e){const t=E("map-axes"),o=E("map-scale-x"),n=E("map-scale-y"),r=E("lyapunov-readout");let a=null,l=0,s=null;function c(){if(l=0,Z(t,e.getView(),o,n),!a){r.textContent=pe(e.params);return}const p=e.clientToWorld(a.x,a.y),m=Math.min(640,e.params.MAX_ITERATIONS??b),f=oe(p.x,p.y,m,e.params.RHYTHM??g,e.params.SEED??A);r.textContent=`A ${p.x.toFixed(5)} · B ${p.y.toFixed(5)} · λ ${ue(f)}`}function i(){l||(l=requestAnimationFrame(c))}return e.clip.addEventListener("pointermove",p=>{p.buttons!==0?a=null:a={x:p.clientX,y:p.clientY},i()}),e.clip.addEventListener("pointerleave",()=>{a=null,i()}),s=D(e.map,e.controls,e.onParamsChange,e.resetTransition,e.signals),{draw:i,tick(){},reset(){a=null,i()},noteActivity(){},resize:i,dismiss:()=>s?.setOpen(!1),pickPoint(){return!1},syncBudget:X}}const me={compositor:ce,mount:fe},de=`// Lyapunov exponent of a periodically forced logistic map.
...
`;function he(e){return new Worker(""+new URL("tile-worker-Dtc1dl78.js",import.meta.url).href,{type:"module",name:e?.name})}let d=null,xe=1,P=0;const _=new Map,M=[],z=[];function L(){const e=typeof navigator>"u"?4:navigator.hardwareConcurrency??4;return Math.max(1,Math.min(te,e-1))}function ge(){return L()}function ye(){const e=M.pop();return e?Promise.resolve(e):new Promise(t=>z.push(t))}function k(e){if(!d?.includes(e))return;const t=z.shift();t?t(e):M.push(e)}function be(e){const t=_.get(e.data.id);if(t){if(_.delete(e.data.id),!e.data.buffer||e.data.error){t.reject(new Error(e.data.error??"Lyapunov CPU worker returned no buffer"));return}t.resolve(e.data.buffer)}}function N(){const e=new he;return e.onmessage=be,e.onerror=t=>V(e,new Error(t.message||"Lyapunov CPU worker failed")),e.onmessageerror=()=>V(e,new Error("Lyapunov CPU worker message error")),e}function V(e,t){for(const[a,l]of _)l.worker===e&&(_.delete(a),l.reject(t));const o=M.indexOf(e);if(o>=0&&M.splice(o,1),!d)return;const n=d.indexOf(e);if(n<0)return;d.splice(n,1);try{e.terminate()}catch{}const r=N();d.push(r),k(r)}function _e(){if(!d)return;P+=1;const e=new Set(d);d=[],M.length=0;const t=new Error("Lyapunov CPU tile superseded by a newer camera");t.name="AbortError";for(const[o,n]of _)e.has(n.worker)&&(_.delete(o),n.reject(t));for(const o of e)try{o.terminate()}catch{}for(let o=0;o<L();o++){const n=N();d.push(n),k(n)}}function ve(){if(!d){d=[];for(let e=0;e<L();e++){const t=N();d.push(t),M.push(t)}}}function Me(e,t){return new Promise((o,n)=>{_.set(t.id,{worker:e,resolve:o,reject:n}),e.postMessage(t)})}async function we(e,t,o,n){ve();const r=P,a=await ye();try{if(r!==P){const s=new Error("Lyapunov CPU tile superseded before dispatch");throw s.name="AbortError",s}const l=await Me(a,{id:xe++,width:Math.max(1,Math.round(t)),height:Math.max(1,Math.round(o)),xMin:e.xMin,xMax:e.xMax,yMin:e.yMin,yMax:e.yMax,maxIter:Math.max(b,Math.round(n.MAX_ITERATIONS??b)),rhythm:Math.round(n.RHYTHM??g),seed:n.SEED??A});return new Float32Array(l)}finally{k(a)}}function Ae(e,t,o,n,r){const a=Math.max(b,Math.round(n.MAX_ITERATIONS??b)),l=new ArrayBuffer(80),s=new Float32Array(l),c=new Uint32Array(l);s[0]=e.xMin,s[1]=e.xMax,s[2]=e.yMin,s[3]=e.yMax,s[4]=n.SEED??A,s[5]=T,s[6]=Y,s[7]=F,c[12]=a,c[13]=t,c[14]=o,c[15]=Math.max(0,Math.min(h.length-1,Math.round(n.RHYTHM??g)));const i=$(e,r.normView??e,t,o);return c[17]=i.y0<<16|i.x0,c[18]=i.y1<<16|i.x1,c[19]=1,l}const Ee={computeWgsl:de,entryPoint:"simulate",uniformBytes:80,tileSize:128,dispatchBatch:1,packUniforms:Ae},Pe={id:"lyapunov",title:"Lyapunov fractal",settledResolution:"device",settledResolutionIdleMs:0,defaultView:{xMin:R,xMax:B,yMin:R,yMax:B},workBudget:{param:"MAX_ITERATIONS",min:b,max:C,step:I,adaptive:!1},params:[{key:"RHYTHM",label:"param.lyapunov.rhythm",kind:"int",min:0,max:h.length-1,step:1,default:g,choices:h,section:"primary"},{key:"MAX_ITERATIONS",label:"param.lyapunov.accuracy",kind:"int",min:b,max:C,step:I,default:ee,section:"primary"},{key:"SEED",label:"param.lyapunov.seed",kind:"float",min:.05,max:.95,step:.01,default:A,digits:2,section:"primary"}],gpu:Ee,cpu:{fillTile:we,cancelPending:_e,concurrency:ge()}};async function Te(){await q(),await j(Pe,me),G(),K()}Te();
